...
public class Sale {
	...
	public Sale(String salesCode, String storeCode, Person customer, Person salesperson, List<Item> items) {
	...
	public void saleReport() {
		...
		System.out.printf("    %4s\n    %4s, %s %s %s\n\n", salesperson.getAddress().getStreet(),
				salesperson.getAddress().getCity(), salesperson.getAddress().getState(),
				salesperson.getAddress().getZip(), salesperson.getAddress().getCountry());
		...
		for (Item i : this.getItems()) {
			...
			if (i.getType().equals("PN") || i.getType().equals("PU")) {
				...
				String employeeName = (((ServiceSale) i).getEmployee().getLastName() + ", "
						+ ((ServiceSale) i).getEmployee().getFirstName());
